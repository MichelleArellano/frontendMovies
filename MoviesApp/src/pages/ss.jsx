...
function Home () {
  ...
  // Call the get movies query from the backend API
  const [getMovies, { dataMovies, errorMovies }] = useLazyQuery(GET_MOVIES)

  if (dataMovies) {
    console.log('data movies', dataMovies)
    console.log('error movies', errorMovies)
    console.log('getMovies movies', getMovies)
  }
  ...
  return (
    <>
      ...
      <div className='flex flex-wrap background justify-around'>
        {dataMovies && dataMovies.getMovies.map(({ _id, title, description, likes, image, date }) => (
